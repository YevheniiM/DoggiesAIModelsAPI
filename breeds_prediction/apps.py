...
class BreedPredictionModel:
    def __init__(self):
        if settings.DEBUG:
            ...
            with default_storage.open(os.path.join(settings.MODELS_PATH, 'breeds.txt'), "r") as f:
                self.class_names = [n.decode("utf-8").strip() for n in f.readlines()]

        model_cache_key = 'breed_prediction_model'
        self.model = cache.get(model_cache_key)

        if self.model:
            print("Found model in the cache, skipping loading...")
        else:
            print("Haven't found model in the cache, loading...")
            ...
            cache.set(model_cache_key, self.model, None)
